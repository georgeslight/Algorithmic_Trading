...
class MultiInputLSTM(nn.Module):
    def __init__(self, input_sz: int, hidden_sz: int, output_days: int):
        """
        ...
        self.fc = nn.Linear(hidden_sz * 5, output_days)
    ...
    def forward(self, open_seq, high_seq, low_seq, close_seq, volume_seq):
        ...
        combined_features = torch.cat(
            (open_hidden[-1], high_hidden[-1], low_hidden[-1], close_hidden[-1], volume_hidden[-1]), dim=1
        )
